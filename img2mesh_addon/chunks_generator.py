...
class ChunksGenerator(object):
    def __init__(self, img_path, calib=(1.0, 1.0, 1.0), obj_diam_yx=30):
        if img_path is None:
            raise ValueError("The provided path should not be None.")
        img_path = Path(img_path)
        if not img_path.exists() or not img_path.is_file():
            raise ValueError("The provided path is not an existing file.")
        self.tiff_path = img_path
    ...
    def _get_shape(self):
        if self.shape is not None:
            return self.shape
        with TiffFile(self.tiff_path) as dsc:
            s = dsc.series[0]
            axes = s.axes
            if axes != "ZYX":
    ...
    def load_by_chunks(self, chunk_size, callback=None):
        ...
        with TiffFile(self.tiff_path) as tf:
            zobj = tf.aszarr(series=0, level=0, chunkmode=CHUNKMODE.PAGE)
        arr = zarr.open(zobj, mode="r")
        Z, Y, X = img_shape
        ...
        for z0 in range(0, Z, step_z):
            for y0 in range(0, Y, step_y):
                for x0 in range(0, X, step_x):
                    ...
                    callback(chunk, (z0, y0, x0), (z1, y1, x1), self.calibration, index)
                    index += 1

def test_nuclei_segmentation():
    from chunk_callbacks import (get_save_chunk_as, get_cellpose_run,
                             get_segment_as_meshes)
    from make_meshes import LabelMeshAccumulator

    path = "/home/clement/Documents/projects/mifobio-2025/flash-tuto/data/tests-callbacks"
    cg = ChunksGenerator(
        "/home/clement/Documents/projects/mifobio-2025/flash-tuto/data/c_elegans/eft3RW10035L1_0125071.tif",
        (0.122, 0.116, 0.116),
        15
    )
    ...
    sca_fx = get_save_chunk_as(Path(path) / "chunks")
    cpr_fx = get_cellpose_run(Path(path) / "cp_masks", cg.get_obj_diam_yx(), cg.get_anisotropy())

    mesh_folder = Path(path) / "mesh"
    sam_fx = get_segment_as_meshes(mesh_folder, cg.get_overlap(), cg.get_anisotropy(), lma) # folder_path, overlap, ani_factor, lma
